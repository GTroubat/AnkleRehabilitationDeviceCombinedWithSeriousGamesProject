...
public class MathematicalModel3 : MonoBehaviour
{
    ...
    // pivotPoint must be a 3D vector (x,y,z) - centre de la plateforme
    ...
    private float[,] baseA = new float[4, 3];
    private float[,] baseB = new float[4, 3];
    private float[,] baseAp = new float[4, 3];
    private float[,] baseApp = new float[4, 3];
    private float[,] baseAf = new float[4, 3];
    ...
    void Start() { 
        ...
        roll = 0;
        yaw = 0;
        baseA = new float[4,3] { { -35 + changeBase[0], 60 + changeBase[1], 80 + changeBase[2] }, 
        ...
        // Use origin as external pivot: rotation of A around (0,0,0)
        pivotPoint = new float[3] { 0f, 0f, 0f };
    }
    ...
    private void FixedUpdate() {
        ...
        baseB = new float[4, 3] { { -28, 150, -285 + height }, {195, 150, -285 + height },
            {195, -150, -285 + height }, {-28, -150, -285 + height } };
        rotationMatrix = EulerRotationMatrix(phi, theta, psi);
        baseAp = SubstractVectorToMatrix(baseA, pivotPoint);
        baseApp = MultiplyMatrices(baseAp, rotationMatrix);
        baseAf = AddVectorToMatrix(baseApp, pivotPoint);
        cableVectors = SubstractTwoMatrix(baseAf, baseB);
    ...
    private float[,] AddVectorToMatrix(float[,] matrix, float[] vector) {
        ...
        // vector must represent a 3D offset (length == number of columns)
        if (vector.Length != cols) {
            throw new InvalidOperationException(
                "Addition Impossible. " +
                "Vector length must be equal to the number of columns in the matrix."
            );
        }
