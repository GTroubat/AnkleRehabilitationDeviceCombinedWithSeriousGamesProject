...
public class AdaptiveRewardAgentV3: Agent {
    [Header("References")]
    ...
    public override void OnActionReceived(ActionBuffers actions) {
        ...
        {
            AddReward(-oscillation * oscillationPenaltyWeight * marblesProcessedInEpisode * (1+consecutiveSuccesses+consecutiveErrors));
        }
        ...
        if (consecutiveSuccesses > 2) {
            ...
            stepReward = - currentAssistanceLevel * adaptationRewardWeight * consecutiveSuccesses;
        } else if (consecutiveErrors > 1) {
            // Le joueur est en difficulté : on veut que l'assistance tende vers 1
            stepReward = currentAssistanceLevel * adaptationRewardWeight * consecutiveErrors;
        } else {
    ...
    public void OnMarbleScored(bool success) {
        ...
        if (success) {
            ...
            float reward = successReward * (1.0f - averageAssistance) * (consecutiveSuccesses + 1) * (consecutiveSuccesses + 1);
            AddReward(reward);
